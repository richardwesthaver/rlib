  ...
  M: Meter<K, V>,
{
  ...
  /// Returns a mutable reference to the value corresponding to the given key in
  /// the cache, if any.
  ...
  /// update the Cache state so the key's position will be unchanged.
  ...
  /// `peek_mut` does not update the Cache state so the key's position will be
  /// unchanged.
  ...
  /// cache is empty. Like `peek`, `peek_by_policy` does not update the Cache
  /// state so the item's position will be unchanged.
  ...
  /// Removes and returns the key-value pair as a tuple by policy (Lru, Lfu,
  /// etc.).
