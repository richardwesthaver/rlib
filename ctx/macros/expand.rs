...
pub fn expand(
  ...
  if function.sig.inputs.len() > 1 {
    return Err(Error::new_spanned(
      function.sig,
      "expected one argument of type ctxinit::CtxInit",
    ));
  }
  ...
  if mode == Mode::Main && function.sig.ident != "main" {
    return Err(Error::new_spanned(
      function.sig,
      "#[ctx::main] must be used on the main function",
    ));
  }
  ...
  let guard = match mode {
    Mode::Main => Some(quote! {
        if module_path!().contains("::") {
            panic!("ctxinit must be performed in the crate root on the main function");
        }
    }),
    Mode::Test => None,
  };
  ...
  let body = match (function.sig.asyncness.is_some(), mode) {
    (true, Mode::Test) => quote! {
        ctx::init_tokio::tokio_test(async #block )
    },
    (true, Mode::Main) => quote! {
        ctx::init_tokio::tokio_main(async #block )
    },
    (false, _) => {
  ...
  let perform_init = match disable_fatal_signals {
    DisableFatalSignals::Default(_) => {
      ...
      quote! {
          ctx::r#impl::perform_init_with_disable_signals(0x8002)
      }
    }
    DisableFatalSignals::All(_) => {
      // ffff is a mask of all 1's
      quote! {
          ctx::r#impl::perform_init_with_disable_signals(0xffff)
      }
    }
    DisableFatalSignals::SigtermOnly(_) => {
      // 8000 is 1 << 15 (SIGTERM)
      quote! {
          ctx::r#impl::perform_init_with_disable_signals(0x8000)
      }
    }
    DisableFatalSignals::None(_) => {
      quote! {
          ctx::r#impl::perform_init()
      }
    }
  };
  ...
  function.block = parse_quote!({
      ...
      let destroy_guard = unsafe { ctx::r#impl::DestroyGuard::new() };
      #body
  });
